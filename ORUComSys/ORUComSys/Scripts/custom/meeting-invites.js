$(document).ready(() => {
    console.log("Script loaded: meeting-invites.js");
});
...
function InviteUser() {
    ...
    if (action == "Invite") {
        SendInvite(UserId, MeetingId);
    } else if (action == "Remove") {
        RemoveInvite(UserId, MeetingId);
    }
}

function SendInvite(UserId, MeetingId) {
    var invite = { MeetingId: MeetingId, ProfileId: UserId };
    $.ajax({
        url: "/Meeting/AddMeetingInvite/",
        type: "POST",
...
function RemoveInvite(UserId, MeetingId) {
    var invite = { MeetingId: MeetingId, ProfileId: UserId };
    $.ajax({
        url: "/Meeting/RemoveMeetingInvite/",
        type: "POST",
        data: JSON.stringify(invite),
        contentType: "application/json;charset=UTF-8",
        success: function (data) {
